...
export default function DialogueChat({ websocketUrl }) {
  // Conversation state
  ...
  const startConversation = () => {
    ...
    ws.onmessage = (event) => {
      ...
      if (msgType === "assistant_delta") {
        ...
        // Update transcript if text is available
        if (payload.text) {
          console.log("Assistant delta:", payload.text);
          setTranscript(prev => {
            // Find the last assistant message that's not final
            const lastAssistantIndex = prev.findIndex(msg => 
              msg.role === "assistant" && !msg.final
            );
            
            if (lastAssistantIndex >= 0) {
              // Update the existing message by appending new text
              const updatedMessages = [...prev];
              updatedMessages[lastAssistantIndex] = {
                ...updatedMessages[lastAssistantIndex],
                content: updatedMessages[lastAssistantIndex].content + payload.text
              };
        ...
        if (payload.text) {
          ...
          setTranscript(prev => {
            // Remove any non-final assistant messages and add the final one
            const withoutPending = prev.filter(msg => msg.role !== "assistant" || msg.final);
            return [
              ...withoutPending,
              { 
