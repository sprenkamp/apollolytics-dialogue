...
"""
    ...
    # Enter a loop to allow multiple conversation turns
    while True:
        user_input = await asyncio.to_thread(input)
        messages.append({"role": "user", "content": user_input})

        # Keep conversation history within the defined message limit
        if len(messages) > MESSAGE_LIMIT + 1:
            messages = [messages[0]] + messages[-MESSAGE_LIMIT:]

        # If there was a previous audio response, include its audio id for continuity
        if last_audio_id:
            messages.append({"role": "assistant", "audio": {"id": last_audio_id}})

        # Request the next assistant response
        completion = client.chat.completions.create(
            model="gpt-4o-audio-preview",
            modalities=["text", "audio"],
            audio={"voice": "alloy", "format": "wav"},
        ...
        # Save the audio response if provided
        if completion.choices[0].message.audio:
            last_audio_id = completion.choices[0].message.audio.id
            wav_bytes = base64.b64decode(completion.choices[0].message.audio.data)
            response_counter += 1
        ...
        # Append assistant's text response to conversation history
        messages.append({"role": "assistant", "content": assistant_response_text})
